...
function scramble(str) {
  return str.split('').map(c => c === ' ' ? ' ' : Math.random() < 0.4 ? randChar() : c).join('');
}
...
export default function Resources() {
  const ciphers = [
    ...
    // step_5 = F10
    // execute_sequence_in_order :: trigger = width_collapse
    // ───────────────────────────────────────────────────────
    { id: '08', data: '' },
    { id: '09', data: '' },
    { id: '10', data: '' },
  ...
  useEffect(() => {
    function check() {
      ...
      if (narrow && phaseRef.current === 'idle') {
        ...
        seqIdxRef.current = 0;
      } else if (!narrow && (phaseRef.current === 'frozen' || phaseRef.current === 'early_warn' || phaseRef.current === 'waiting')) {
        resetAll();
      }
    }
  ...
  useEffect(() => {
    ...
    if (phase === 'frozen' || phase === 'early_warn' || phase === 'waiting') {
      ...
      el.textContent = `* { cursor: none !important; pointer-events: none !important; } #unfreeze-overlay, #unfreeze-overlay * { pointer-events: all !important; }`;
    } else {
      if (el) el.remove();
    }
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (p !== 'frozen' && p !== 'early_warn' && p !== 'waiting') return;
  ...
  function startScramble(base) {
    ...
    scrambleInt.current = setInterval(() => {
      setScrambledMsg(scramble(base));
    }, 60);
  }
  ...
  function Overlay() {
    ...
    if (overlay === 'early') return (
      <div style={wrapStyle} id="unfreeze-overlay">
        <div style={scanStyle} />
        <div style={{
          ...boxBase,
          border: '1px solid #ff2a1f',
          animation: 'shakeBox 0.1s infinite',
        }}>
          {staticBar}
          {staticBar}
          <div style={{
            fontSize: 20, letterSpacing: 4, fontWeight: 700, color: '#ff2a1f',
            animation: 'chaosGlitch 0.15s infinite',
          }}>⚠ SYSTEM UNSTABLE</div>
          {staticBar}
          <div style={{
            fontSize: 11, letterSpacing: 3, color: '#ff2a1f',
            animation: 'flicker 0.3s infinite',
          }}>{scrambledMsg}</div>
          <div style={{
            fontSize: 9, letterSpacing: 2, color: '#ff2a1f', opacity: 0.6,
            animation: 'blink 0.4s step-end infinite',
          }}>signal destabilised — stand by —</div>
          {staticBar}
          <div style={{
            fontSize: 9, color: '#ff2a1f', letterSpacing: 2,
            animation: 'flicker 0.2s infinite',
          }}>ERR_0x{Math.floor(Math.random() * 9999).toString(16).toUpperCase()}</div>
          {staticBar}
        </div>
      </div>
    );
    ...
    if (overlay === 'pattern') return (
      <div style={wrapStyle} id="unfreeze-overlay">
        ...
        <div style={{
          ...
          {staticBar}
          <div style={{
            fontSize: 18, letterSpacing: 4, fontWeight: 700, color: '#ff6b00',
            animation: 'chaosGlitch 0.2s infinite',
          }}>⚠ SYSTEM STILL UNSTABLE</div>
          {staticBar}
          <div style={{ fontSize: 11, letterSpacing: 3, color: '#ff6b00' }}>
            DRAW PATTERN TO RESTORE
          </div>
          <div style={{
            fontSize: 9, letterSpacing: 2, color: '#ff6b00', opacity: 0.6,
            animation: 'blink 0.5s step-end infinite',
          }}>connect the nodes — restore signal</div>
          {staticBar}
          <div
            style={{
              display: 'grid', gridTemplateColumns: 'repeat(3,1fr)',
              gap: 8, marginTop: 8, userSelect: 'none',
            }}
    ...
    if (overlay === 'unlocked') return (
      <div style={wrapStyle} id="unfreeze-overlay">
        ...
        <div style={{ ...boxBase, border: '1px solid #39ff14' }}>
          <div style={{ fontSize: 18, letterSpacing: 4, fontWeight: 700, color: '#39ff14' }}>
            ✓ SYSTEM STABILISED
          </div>
          ...
          <div style={{
            ...
            color: '#39ff14', fontSize: 11, lineHeight: 2,
            letterSpacing: 1, textAlign: 'left', width: '100%',
          }}>
            REQUIREMENT: Implement a mini game.<br />
            <span style={{ fontSize: 9, color: '#2a7a2a' }}>
              It must be playable. A winning condition must exist.
            </span>
          </div>
          <button
            onClick={resetAll}
